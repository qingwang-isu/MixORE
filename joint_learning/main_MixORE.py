...
parser.add_argument('--num-cluster', default='16,32,39,64', type=str,
...
parser.add_argument('--exp-dir', default='retacred_mix_half_novel_added_continual', type=str,
...
def main_worker(gpu, args):
    ...
    sentence_train = json.load(open(args.data + 'retacred_train_sentence_half.json', 'r'))
    sentence_train_label = json.load(open(args.data + 'retacred_train_label_half.json', 'r'))
    sentence_train_must_links = json.load(open(args.data + 'retacred_half_train_pairs.json', 'r'))
    #sentence_train = json.load(open(args.data + 'fewrel_train_sentence_top_half.json', 'r'))
    #sentence_train_label = json.load(open(args.data + 'fewrel_train_label_top_half.json', 'r'))
    #sentence_train_must_links = json.load(open(args.data + 'fewrel_top_half_train_pairs_3.json', 'r'))
    print(len(sentence_train))
    print(len(sentence_train_must_links))
    print()
    train_dataset, eval_dataset = pre_processing(sentence_train, sentence_train_label, sentence_train_must_links, args)
    ...
    eval_loader = torch.utils.data.DataLoader(
        eval_dataset, batch_size=args.batch_size, shuffle=False, num_workers=args.workers)
    ...
    print()
    print("processing continual learning data ...")
    sentence_train_new = json.load(open(args.data + 'retacred_train_sentence_half_novel_added.json', 'r'))
    sentence_train_label_new = json.load(open(args.data + 'retacred_train_label_half_novel_added.json', 'r'))
    sentence_train_must_links_new = json.load(open(args.data + 'retacred_half_novel_added_train_pairs.json', 'r'))
    print(len(sentence_train_new))
    print(len(sentence_train_must_links_new))
    print()
    train_dataset_new, eval_dataset_new = pre_processing(sentence_train_new, sentence_train_label_new, sentence_train_must_links_new, args)


    train_loader_new = torch.utils.data.DataLoader(
        train_dataset_new, batch_size=args.batch_size, shuffle=True,
        num_workers=args.workers, drop_last=True)

    # dataloader for center-cropped images, use larger batch size to increase speed
    eval_loader_new = torch.utils.data.DataLoader(
        eval_dataset_new, batch_size=args.batch_size, shuffle=False, num_workers=args.workers)

    gold_new = []
    for eval_data_new in eval_dataset_new:
        gold_new.append(eval_data_new[2].item())
    ...
    for epoch in range(args.start_epoch, args.epochs):
        if epoch <= 1:
            ...
            # train for one epoch
            # train with only existing relation labels
            ...
            if epoch >= args.warmup_epoch:
                measure_file.write('Epoch :' + str(epoch) + '\n')
                # compute momentum features for center-cropped images
                features = compute_features(eval_loader_new, model, args)

                # placeholder for clustering result
                cluster_result = {'relation2cluster': [], 'centroids': [], 'density': []}
                for num_cluster in args.num_cluster:
                    cluster_result['relation2cluster'].append(torch.zeros(len(eval_dataset_new), dtype=torch.long).cuda())
                    cluster_result['centroids'].append(torch.zeros(int(num_cluster), args.low_dim).cuda())
                    cluster_result['density'].append(torch.zeros(int(num_cluster)).cuda())
                ...
                if args.gpu == 0:
                    ...
                    print('{} Running time: {} Seconds'.format(run_which, cluster_end - cluster_start))
                    measure_file.write(
                        '{} epoch:{} Running time: {} Seconds\n'.format(run_which, epoch, cluster_end - cluster_start))
                    # saving cluster measurements
                    calculate_measurements(cluster_result, args, gold_new, measure_file, features)

            adjust_learning_rate(optimizer, epoch, args)
            # continual learning, with novel added
            epoch_loss = train(train_loader_new, model, criterion, criterion_margin, optimizer, epoch, args, cluster_result)
...
def train(train_loader, model, criterion, criterion_margin, optimizer, epoch, args, cluster_result=None):
    ...
    for i, batch in enumerate(train_loader):
        ...
        # compute output
        output, target, ml_triplet_lst, output_proto, target_proto, output_c = model(sen_q=batch[:6], sen_k=batch[:5]+[batch[6]], cluster_result=cluster_result, index=batch[7], print_index=i, must_links_l=batch[8:13], must_links_r=batch[13:17]+[batch[-1]])
        ...
        # Must-link averaged triplet loss
        ...
        gt_labels = batch[2]
        # cross-entropy class prediction loss
        loss_c = criterion(output_c, gt_labels)
        loss += loss_c
...
def pre_processing(sentence_train, sentence_train_label, sentence_train_must_links, args):
    ...
    for i in range(len(sentence_train)):
        ...
        try:
            ...
            for j in range(len(sentence_train_must_links[i])):
                ...
                # Find e1(id:2487) and e2(id:2475) position
                l_ml_e1_tks_id, l_ml_e2_tks_id = get_token_word_id(sentence_train_must_links[i][j][0], tokenizer, args)
                ...
                # data augmentation
                l_ml_aug_pos = word_level_augmentation(l_ml_pos1, l_ml_pos2, l_ml_encoded_dict['input_ids'].nonzero().shape[0], args)
                ...
                # Find e1(id:2487) and e2(id:2475) position
                ml_e1_tks_id, ml_e2_tks_id = get_token_word_id(sentence_train_must_links[i][j][1], tokenizer, args)
                ...
                # data augmentation
                ml_aug_pos = word_level_augmentation(ml_pos1, ml_pos2, ml_encoded_dict['input_ids'].nonzero().shape[0], args)
